...
<artifacts>
- `.planning/STATE.md` — populated with discovery answers
</artifacts>
...
<entry>
Discovery phase complete. Or user runs `/dockit:add-section`.
</entry>

<activities>

- Present the section checklist for the chosen documentation type — reference `section-checklists.md`
...
<artifacts>
- `.planning/docs/{doc-name}/structure.md` — chosen sections and order
- `.planning/docs/{doc-name}/checklist.md` — per-section completion tracking (all sections start at `pending`)
- `docs/{doc-name}.md` — document file with all section headings
</artifacts>
...
<transition>
Prompt the user to run `/dockit:continue` to begin drafting.
</transition>
...
<entry>
User runs `/dockit:continue`. The user selects which section they want to work on from the list of sections that are in `pending` state.
</entry>
...
2. Ask the guiding questions for that section (from `section-checklists.md`)
...
<entry>
User runs `/dockit:revision`. The user selects which section they want to revise from the list of sections that are in `draft` state.
</entry>
...
<transition>
The user chooses what to do next — revise another section, draft a pending section, or move to review when all sections are `revised`. Prompt the user to run `/dockit:review` once every section has reached `revised` state.
</transition>
...
1. **Voice reviewer** — checks against the style guide at `.claude/dockit/style-guide.md`
...
<artifacts>
- `.planning/reviews/{doc-name}-review.md` — consolidated review report
</artifacts>
...
`STATE.md` tracks both the document-level status and per-section progress:
...
- **Sections**:
  ...
  - Examples: pending
  - Properties: revised
- **Progress**: 1/7 complete, 3/7 revised, 1/7 in-review, 1/7 draft, 1/7 pending
- **Next**: Waiting for review results on "Usage"
```
