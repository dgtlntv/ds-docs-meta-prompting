...
The deterministic score provides the baseline. The LLM adds nuance that formulas can't capture.
...
Not all passive voice is bad — sometimes it's appropriate (especially in accessibility requirements). Flag it but note when it might be intentional.
...
- Must be expanded on first use: "Web Content Accessibility Guidelines (WCAG)"
- After first expansion, acronym alone is fine
- Well-known exceptions (HTML, CSS, URL, API) don't need expansion in developer docs
