...
import { getDefaultSettings, spellCheckDocument, } from "cspell-lib";
// ============================================================================
...
const FLAGGED_WORDS_PATH = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..", "dockit", "references", "flagged-words.txt");
/**
 ...
 */
function loadFlaggedWords(filePath) {
 ...
 */
async function checkSpelling(text, filePath, forbiddenWords) {
 ...
 */
export async function analyseFile(filePath) {
    ...
    const singleWordEntries = flaggedEntries.filter((e) => !e.word.includes(" "));
    ...
    const { spelling, flagged } = await checkSpelling(text, resolvedPath, allFlagWords);
 ...
 */
function deduplicateForbidden(issues) {
    const seen = new Set();
...
//# sourceMappingURL=copy-editing.js.map