...
2. Run Phase 1 (Discovery) as described in @.agents/dockit/references/phases.md. Use @.agents/dockit/references/documentation-types.md for the type taxonomy and @.agents/dockit/references/tiers.md for tier selection. If the user provides a Figma URL or mentions existing designs, use the `figma-context` skill.
3. Populate `.planning/STATE.md` following the template in @.agents/dockit/templates/state.md. Set the phase to `Structure`.
4. Run Phase 2 (Structure) as described in @.agents/dockit/references/phases.md. Use @.agents/dockit/references/sections.md for the available sections.
5. Create `.planning/docs/{doc-name}/structure.md` following the template in @.agents/dockit/templates/structure.md.
6. Create `.planning/docs/{doc-name}/checklist.md` following the template in @.agents/dockit/templates/checklist.md.
7. Create `docs/{doc-name}.md` following the template in @.agents/dockit/templates/document.md.
...
- **Review** — if a review report exists at `.planning/reviews/{doc-name}-review.md`, present the findings and enter Phase 6 (Polish) as described in @.agents/dockit/references/phases.md. If no review report exists, prompt the user to run `/dockit:review`.
...
- If there are `pending` sections — offer to vomit-draft one. Follow Phase 3 (Vomit drafting) in @.agents/dockit/references/phases.md. Use @.agents/dockit/references/sections.md for the guiding questions. If a Figma URL is recorded in `STATE.md` and the section would benefit from design context, use the `figma-context` skill.
- If there are `draft` sections — offer to revise one. Follow Phase 4 (Revision) in @.agents/dockit/references/phases.md. Use @.agents/dockit/references/style-guide.md for the revision target.
