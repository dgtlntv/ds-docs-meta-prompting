...
Runs all review agents against a document and presents consolidated findings. Covers Phase 5 (Review) and Phase 6 (Polish) from @.agents/dockit/references/phases.md.
...
1. **Voice reviewer** — uses the `brand-voice-evaluation` skill and @.agents/dockit/references/style-guide.md
2. **Readability reviewer** — uses the `readability-evaluation` skill
3. **Completeness reviewer** — uses the `completeness-evaluation` skill and @.agents/dockit/references/sections.md
...
Collect results from all reviewers and merge them into a single review report. Write the report to `.planning/reviews/{doc-name}-review.md`, following the template in @.agents/dockit/templates/review-report.md.
...
Enter Phase 6 (Polish) as described in @.agents/dockit/references/phases.md. For each finding, the user decides how to handle it: skip, fix now, send back to drafting, or send back to revision.
